...
1. **WakaTime** — физическое время за день. Источник: **дашборд WakaTime** (не CLI).
   > CLI (`~/.wakatime/wakatime-cli --today`) в WSL видит только WSL-активность (~40% от реального). Windows VS Code и браузер отправляют данные отдельно. Правильный источник — дашборд или спросить пользователя.
