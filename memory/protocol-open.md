...
**Шаг 3.** Определить файлы/репо, ограничения. Если есть context file (`DS-strategy/inbox/WP-{N}*.md`) — прочитать. Если нужен доменный контекст → `knowledge-mcp search("тема")`.

> **Критичное чтение:** Context file может устареть. Иерархия доверия: (1) реальные файлы/код → (2) MCP search (свежие данные) → (3) WP context / WORKPLAN.
