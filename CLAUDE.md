...
- **Done РП**: зачёркнуть всю строку обычным шрифтом, без bold (`| ~~#~~ | ~~название~~ | ... |`). **Тильда внутри `~~`:** символ `~` внутри strikethrough ломает рендер GitHub (интерпретируется как конец `~~`). При зачёркивании заменять `~30 мин` → `30 мин`, `{{WORKSPACE_DIR}}/` → `IWE/`.
