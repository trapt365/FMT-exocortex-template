...
#
# Пример:
#   ./transcribe.sh "/mnt/c/Users/Timur/iCloudDrive/iCloud~com~openplanetsoftware~just-press-record/2026-04-03/18-06-08.m4a" "dieselgen разговор с Игорем 03 апреля 2026"
#
# Результат: Obsidian-заметка в Vault1 с диаризацией

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ENV_FILE="$SCRIPT_DIR/../.exocortex.env"
VAULT_DIR="/mnt/c/Users/Timur/Documents/Vault1"
JUST_PRESS_RECORD="/mnt/c/Users/Timur/iCloudDrive/iCloud~com~openplanetsoftware~just-press-record"

# Load API key
if [ ! -f "$ENV_FILE" ]; then
    echo "ERROR: $ENV_FILE not found. Create it with DEEPGRAM_API_KEY=..."
    exit 1
fi
source "$ENV_FILE"
...
lines = []
lines.append('---')
lines.append(f'created: $(date -r "$AUDIO_FILE" "+%Y-%m-%dT%H:%M:%S%:z" 2>/dev/null || date "+%Y-%m-%dT%H:%M:%S%:z")')
