...
function getToken() {
    ...
    if (!token) {
        throw new Error("SINGULARITY_API_TOKEN not set. Get it from me.singularity-app.com → «Доступ AI и API»");
    }
    return token;
}
...
function formatTask(task, projectName) {
    const parts = [];
    ...
    if (task.tags?.length)
        parts.push(`  Теги: ${task.tags.join(", ")}`);
    if (task.note)
        parts.push(`  Заметка: ${task.note.slice(0, 200)}`);
    return parts.join("\n");
}
// --- Server ---
...
server.tool("list-tasks-today", "Список задач из SingularityApp на сегодня (по дате start). Возвращает задачи, запланированные на текущий день.", {}, async () => {
    ...
    if (active.length === 0) {
        return {
            content: [
                { type: "text", text: `Задач на ${dateStr} не найдено.` },
            ],
        };
    }
    // Resolve project names
    const lines = [`## Задачи на ${dateStr} (${active.length})\n`];
...
// Tool: list-projects
server.tool("list-projects", "Список проектов из SingularityApp. Опционально фильтрует по подстроке в названии.", {
    filter: z
        .string()
        .optional()
    ...
    if (filter) {
        ...
        projects = projects.filter((p) => p.title.toLowerCase().includes(lower));
    }
    ...
    const lines = [`## Проекты (${projects.length})\n`];
...
// Tool: get-task
server.tool("get-task", "Получить детали задачи по ID из SingularityApp.", {
...
// Tool: list-tasks
server.tool("list-tasks", "Список задач из SingularityApp с фильтрами по проекту и дате.", {
    projectId: z
        .string()
        .optional()
    ...
    const data = await apiRequest("/task", params);
